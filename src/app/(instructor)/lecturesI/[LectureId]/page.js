...
if(userToken&&LectureId){
  ...
  <div className="tab-content" id="myTabContent">
    ...
    <Box
      ...
      >
<Button sx={{px:2}} variant="contained" className='buttonAdd' startIcon={<AddCircleOutlineIcon />} onClick={handleClickOpen}>
  Add New
</Button>
      </Box>
      <Dialog
...
{contents.map(({ icon: Icon, type})=>( 
  <Button onClick={()=>handleAddContent(type)}>
   ...
   spacing={1}
    >
      <div className='item'>
     <Icon sx={{fontSize:50, }} className='itemmm'/>
     <Typography variant='h6' className='itemmm'> {type}</Typography>
     </div>
     </Stack>
     </Button>
))}
